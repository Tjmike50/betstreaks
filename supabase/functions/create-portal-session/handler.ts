...
export async function handlePortalRequest(req: Request, dependencies: {
  ...
  try {
    const supabaseClient = makeClient(supabaseUrl, supabaseAnonKey, {
    ...
    if (userError || !user) {
      console.error("[create-portal-session] authentication failed");
      return jsonOk({ error: "Not authenticated.", code: "no_auth" });
    }

    const supabaseAdmin = makeClient(supabaseUrl, supabaseServiceRoleKey, {
      auth: { persistSession: false },
    });

    // Portal stays live-only, even while sandbox checkout is enabled.
    // ── Gather subscription evidence from both live accounts ──
    ...
    console.error("[create-portal-session] portal creation failed");
    return jsonOk({
